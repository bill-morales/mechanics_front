...
const generateForm = (marca: Imarca | null = null) => {
    return defineForm({
        name: field(marca?.name || '', yup.string().required('Nombre requerido')),
    })
}
const submitted = ref(false);
const submitting = ref(false);
const form = shallowRef(generateForm(props.data.marca));
...
const handleSubmit = async () => {
    ...
    const marca: Imarca = toObject(form.value);
    marca.id = props.data.marca?.id;
    try {
     props.data.marca ?  await marcaService.updateMarca(marca) : await marcaService.createMarca(marca)
